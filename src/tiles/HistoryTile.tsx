        ...
        {recent.length === 0 ? <span className="tile__empty">{t('history.noHistory')}</span> : recent.map((entry, i) => {
          ...
          return (
            <div key={i} className="tile__member-row">
              <div className="tile__avatar" style={{ backgroundColor: color }}>{entry.memberIds.length > 1 ? entry.memberIds.length : getInitials(getMember(entry.memberIds[0])?.name || '?')}</div>
              <span className="tile__member-name" style={{ fontSize: 12 }}>{names}</span>
              <span className="tile__duration">{fmtDur(entry.startTime, entry.endTime)}</span>
            </div>
          );
        })}
      </div>
