...
class MsgpackDecoder {
  ...
  private rdU8() { return this.bytes[this.pos++]; }
  private rdU16() { const v = this.view.getUint16(this.pos); this.pos += 2; return v; }
  private rdU32() { const v = this.view.getUint32(this.pos); this.pos += 4; return v; }
  private rdU64() { const hi = this.view.getUint32(this.pos); const lo = this.view.getUint32(this.pos + 4); this.pos += 8; return hi * 4294967296 + lo; }
  private rdI64() { const hi = this.view.getInt32(this.pos); const lo = this.view.getUint32(this.pos + 4); this.pos += 8; return hi * 4294967296 + lo; }
  private readStr(len: number) { const s = utf8Decode(this.bytes, this.pos, len); this.pos += len; return s; }
  private readBin(len: number) { const slice = this.bytes.subarray(this.pos, this.pos + len); this.pos += len; return slice; }
  private readArray(len: number): any[] { const out = new Array(len); for (let i = 0; i < len; i++) out[i] = this.decode(); return out; }
  private readMap(len: number): Record<string, any> {
    const out: Record<string, any> = {};
    for (let i = 0; i < len; i++) { const k = this.decode(); const v = this.decode(); out[typeof k === 'string' ? k : String(k)] = v; }
    return out;
  }
  private readExt(type: number, len: number): any {
    if (type === -1) return this.readTimestamp(len);
    const data = this.bytes.subarray(this.pos, this.pos + len); this.pos += len; return { __ext: type, data };
  }
  private readTimestamp(len: number): number {
    if (len === 4) { const sec = this.view.getUint32(this.pos); this.pos += 4; return sec * 1000; }
    if (len === 8) { const hi = this.view.getUint32(this.pos); const lo = this.view.getUint32(this.pos + 4); this.pos += 8; const nsec = hi >>> 2; const sec = (hi & 0x3) * 4294967296 + lo; return sec * 1000 + Math.floor(nsec / 1e6); }
    if (len === 12) { const nsec = this.view.getUint32(this.pos); const shi = this.view.getInt32(this.pos + 4); const slo = this.view.getUint32(this.pos + 8); this.pos += 12; const sec = shi * 4294967296 + slo; return sec * 1000 + Math.floor(nsec / 1e6); }
    this.pos += len; return 0;
  }
  decode(): any {
    const b = this.rdU8();
    if (b <= 0x7f) return b;
...
export interface AmparTables { [table: string]: any[]; }

export const parseAmparBytes = (bytes: Uint8Array): AmparTables => {
  ...
  while (!dec.eof()) {
    let rec: any;
    try { rec = dec.decode(); } catch { break; }
...
export const parseAmpar = (b64: string): AmparTables => parseAmparBytes(base64ToBytes(b64));
