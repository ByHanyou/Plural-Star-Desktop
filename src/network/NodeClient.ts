...
const bridgeFetch = async (
  ...
  return Promise.race([window.electronAPI.net.fetch(url, options), timeout]);
};
...
export class NodeClient {
  ...
  async health(): Promise<any> {
    const res = await bridgeFetch(`${this.relayUrl}/health`, { headers: this.authHeaders() });
    if (!res.ok) throw new Error(`health ${res.status}`);
    return parseJson(res.text);
  }

  async peers(): Promise<any> {
    const res = await bridgeFetch(`${this.relayUrl}/peers`, { headers: this.authHeaders() });
  ...
  async send(
    ...
    const res = await bridgeFetch(`${this.relayUrl}/send`, {
      method: 'POST',
      headers: this.authHeaders(),
      body: JSON.stringify(body),
    });
    if (!res.ok) throw new Error(`send ${res.status} ${res.text || ''}`);
    return parseJson(res.text) as SendResult;
  }
  ...
  async rendezvousRegister(
    ...
    const res = await bridgeFetch(`${this.relayUrl}/rendezvous/register`, {
      method: 'POST',
      headers: this.authHeaders(),
